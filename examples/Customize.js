...
const Image = ({
  ...
  <div style={{ ...style.image, ...(selected ? style.selected : {}) }}>
    ...
    {!done && !error && (
      <div style={{ ...style.progress, width: `${progress}%` }} />
    )}
    ...
    <div style={style.controls}>
      <button onClick={select} style={style.selectBTN}>
        Select
      </button>
      <button onClick={remove} style={style.removeBTN}>
        Remove
      </button>
    </div>
  </div>
...
export default class App extends Component {
  render() {
    return (
      <RUG action="/api/upload" header={false} rules={{ limit }}>
        {(images, { openDialogue }) => {
            ...
            <DropArea>
              {isDrag => (
                <div
                  style={{
                    ...style.content,
                    ...(isDrag ? { borderColor: "rgb(0, 122, 255)" } : {})
                  }}
                >
                  <div style={style.header}>
                    <button style={style.button} onClick={openDialogue}>
                      Select Images
                    </button>
                  ...
                  {length ? (
                    <DragArea style={style.imageArea}>
                      {image => (
                        <div>
                          <Image {...image} />
                        </div>
